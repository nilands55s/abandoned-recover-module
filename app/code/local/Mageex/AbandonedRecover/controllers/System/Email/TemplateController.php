...
class Mageex_AbandonedRecover_System_Email_TemplateController extends Mage_Adminhtml_System_Email_TemplateController
{
...
*/
    ...
    public function defaultTemplateAction()
    {
		...
		$optionArray = $template->getVariablesOptionArray(true);
		if(Mage::helper('abandonedrecover/config')->isEnable()){
			
			$arr = array(
				'value' => '{{block type="abandonedrecover/crosssell" customer=$abandoned_customer }}',
				'label' => Mage::helper('core')->__('%s', 'Bock Abandoned Recover')
			);
			array_unshift($optionArray['value'], $arr);
			
		}
        $template->setData('template_variables', Zend_Json::encode($optionArray));
